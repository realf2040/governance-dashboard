...
export default {
 async fetch(request,env){
  ...
  try{
   ...
   if(url.pathname==='/api/users/reset-password'&&request.method==='PUT'){const b=await request.json();if(!b.username||!b.password)return json({ok:false,error:'Required'},400);await env.DB.prepare('UPDATE governance_users SET password=?,updated_at=CURRENT_TIMESTAMP WHERE username=?').bind(String(b.password),String(b.username)).run();return json({ok:true})}
