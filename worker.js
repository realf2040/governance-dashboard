...
export default {
 async fetch(request,env){
  ...
  try{
   await init(env.DB);
   if(url.pathname==='/api/auth/login'&&request.method==='POST'){const b=await request.json(),u=await env.DB.prepare('SELECT username,role FROM governance_users WHERE username=? AND password=? AND active=1').bind(String(b.username||'').trim(),String(b.password||'')).first();if(u){await audit(env.DB,u.username,'LOGIN',null,{role:u.role});return json({ok:true,user:u})}return json({ok:false,error:'Invalid credentials'},401)}
